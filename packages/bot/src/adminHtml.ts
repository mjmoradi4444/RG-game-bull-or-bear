...
// ---- Tasks (catalog CRUD + manual submissions) ----
VIEWS.tasks=function(){main.innerHTML='<h2>Task Management &amp; Submissions</h2><p class="muted">Configure tasks, set reward type (Tokens vs RP) and verification method, or approve pending manual task submissions.</p><div class="toolbar"><button class="btn primary" id="addtkbtn">+ Create New Task</button><button class="btn" id="refreshtkbtn">Refresh</button></div><h3>Pending Manual Submissions</h3><div id="subbody">Loading…</div><h3 style="margin-top:20px">Task Catalog</h3><div id="tkbody">Loading…</div>';document.getElementById('addtkbtn').onclick=showAddTk;document.getElementById('refreshtkbtn').onclick=function(){loadSubmissions();loadTK();};loadSubmissions();loadTK();};
function loadSubmissions(){api('/tasks/submissions').then(function(d){var b=document.getElementById('subbody');if(!b)return;if(!d.submissions||!d.submissions.length){b.innerHTML='<p class="muted">No pending manual submissions requiring review. 🎉</p>';return;}var rows=d.submissions.map(function(s){return '<tr><td><b>'+esc(s.name)+'</b> <span class="muted">(TG '+s.u+')</span></td><td>'+esc(s.taskTitle)+' <span class="muted">('+esc(s.taskId)+')</span></td><td><span class="pill gold">'+s.rewardAmount+' '+esc(s.rewardType)+'</span></td><td><button class="btn gold" data-app-u="'+s.u+'" data-app-task="'+esc(s.taskId)+'">Approve ✓</button> <button class="btn danger" data-rej-u="'+s.u+'" data-rej-task="'+esc(s.taskId)+'">Reject ✕</button></td></tr>';}).join('');b.innerHTML='<table><thead><tr><th>User</th><th>Task</th><th>Reward</th><th>Actions</th></tr></thead><tbody>'+rows+'</tbody></table>';[].forEach.call(b.querySelectorAll('[data-app-u]'),function(bt){bt.onclick=function(){var u=bt.getAttribute('data-app-u');var task=bt.getAttribute('data-app-task');api('/tasks/submissions/'+u+'/'+task+'/approve',{method:'POST',body:'{}'}).then(function(res){if(res.ok){loadSubmissions();loadTK();}else alert('Approval failed: '+(res.error||'error'));});};});[].forEach.call(b.querySelectorAll('[data-rej-u]'),function(bt){bt.onclick=function(){var u=bt.getAttribute('data-rej-u');var task=bt.getAttribute('data-rej-task');api('/tasks/submissions/'+u+'/'+task+'/reject',{method:'POST',body:'{}'}).then(function(res){if(res.ok){loadSubmissions();loadTK();}else alert('Rejection failed: '+(res.error||'error'));});};});});}
function showAddTk(){var id=prompt('Task ID (unique string, e.g. custom_follow_channel):');if(!id)return;var title=prompt('Task Title:', 'Follow our channel');if(!title)return;var rewardType=confirm('Click OK for Token reward, Cancel for RP reward')?'token':'rp';var rewardAmount=Number(prompt('Reward Amount:', rewardType==='token'?'1':'50'))||1;var verifyMethod=prompt('Verify Method (server, tg_member, click_claim, manual):', 'manual')||'manual';var url=prompt('Target URL or channel link (optional):','')||'';api('/tasks/create',{method:'POST',body:JSON.stringify({id:id,title:title,rewardType:rewardType,rewardAmount:rewardAmount,verifyMethod:verifyMethod,url:url,active:true})}).then(function(x){if(x.ok){loadSubmissions();loadTK();}else alert('Failed to create task');});}
...
var vmOpt=['server','tg_member','click_claim','manual','referral'].map(function(v){return '<option value="'+v+'" '+(t.verifyMethod===v?'selected':'')+'>'+v+'</option>';}).join('');
var rtOpt=['rp','token'].map(function(r){return '<option value="'+r+'" '+(t.rewardType===r?'selected':'')+'>'+r+'</option>';}).join('');
return '<tr data-id="'+t.id+'"><td><b>'+esc(t.id)+'</b></td><td><input data-f="title" value="'+esc(t.title)+'" style="width:170px"></td><td><select data-f="verifyMethod" style="width:110px">'+vmOpt+'</select></td><td><input data-f="rewardAmount" type="number" value="'+t.rewardAmount+'" style="width:55px"> <select data-f="rewardType" style="width:75px">'+rtOpt+'</select></td><td><input data-f="'+(t.verifyMethod==="tg_member"?"channel":"url")+'" value="'+esc(t.channel||t.url||"")+'" placeholder="'+(t.verifyMethod==="tg_member"?"@channel":"https://…")+'" style="width:140px"></td><td><input data-f="active" type="checkbox" '+(t.active?"checked":"")+'></td><td class="muted">'+t.completed+' done · '+t.claimed+' claimed</td><td><button class="btn" data-save="'+t.id+'">Save</button></td></tr>';
}).join('');
b.innerHTML='<table><thead><tr><th>ID</th><th>Title</th><th>Method</th><th>Reward</th><th>URL / Channel</th><th>Active</th><th>Funnel</th><th></th></tr></thead><tbody>'+rows+'</tbody></table>';
